...
from quant.partition_data_utils import save_grad_data, save_inp_oup_data


def partition_reconstruction(model: QuantModel, partition: list, cali_data: torch.Tensor,
    ...
    names = [n for n, _ in partition]
    for name, part in partition:
        part.set_quant_state(True, act_quant)
    ...
    if not include_act_func:
        for name, p in partition:
            org_act_func = p.activation_function
            p.activation_function = StraightThrough()
    ...
    loss_func = LossFunction(partition, round_loss=loss_mode, weight=weight, max_count=iters, rec_loss=rec_loss,
                             b_range=b_range, decay_start=0, warmup=warmup, p=p)

    # Save data before optimizing the rounding
    cached_inps, cached_outs = save_inp_oup_data(model, hull, cali_data, asym, act_quant, batch_size)
    if opt_mode != 'mse':
        cached_grads = save_grad_data(model, hull, cali_data, act_quant, batch_size=batch_size)
    else:
        cached_grads = None
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, grad=None):
        ...
        if self.count < self.loss_start or self.round_loss == 'none':
            ...
            round_loss = 0
            for name, module in self.partition:
                if isinstance(module, QuantModule):
