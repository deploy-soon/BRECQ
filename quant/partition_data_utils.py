...
def save_inp_oup_data(model: QuantModel, layer: torch.nn.Module, cali_data: torch.Tensor,
...
def save_grad_data(model: QuantModel, layer: torch.nn.Module, cali_data: torch.Tensor,
...
class GetLayerInpOut:
    def __init__(self, model: QuantModel, layer: torch.nn.Module,
        ...
        if len(layer) == 1:
            ...
            self.data_saver2 = DataSaverHook(store_input=False, store_output=True, stop_forward=True)

    def __call__(self, model_input):
        self.model.eval()
        self.model.set_quant_state(False, False)
        if len(self.layer) == 1:
            handle2 = self.layer[-1].register_forward_hook(self.data_saver2)
        else:
            handle1 = self.layer[0].register_forward_hook(self.data_saver1)
            handle2 = self.layer[-1].register_forward_hook(self.data_saver2)
        with torch.no_grad():
            try:
            ...
            if self.asym:
                ...
                self.data_saver2.store_output = False
                self.model.set_quant_state(weight_quant=True, act_quant=self.act_quant)
                try:
                    _ = self.model(model_input.to(self.device))
                except StopForwardException:
                    pass
                self.data_saver2.store_output = True
...
class GetLayerGrad:
    ...
    def __call__(self, model_input):
        ...
        handle.remove()
        self.model.set_quant_state(False, False)
        #move to partition_recon
        #self.layer.set_quant_state(True, self.act_quant)
        self.model.train()
        return self.data_saver.grad_out.data


def quantize_model_till(model: QuantModule, layer: torch.nn.Module, act_quant: bool = False):
    """
